...
Pin("Y1",Pin.OUT_PP).low()#GND
Pin("Y3",Pin.OUT_PP).high()#VCC
pyb.delay(100)
DQ=DS18X20(Pin('X10'))#DQ
while True:
    tem = DQ.read_temp()
    print('Tem:',tem)
    print(pyb.freq())
    if(run==1):
        ...
        if(num!=0):
            ...
            lcd_5110.lcd_write_string('TEM:',0,3)
            lcd_5110.lcd_write_string(str(tem),25,3)
            lcd_5110.lcd_write_string('C',75,3)
        flag=0
        run=0